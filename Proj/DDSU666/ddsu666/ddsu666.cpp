...
DDSU666Reader::DDSU666Reader(const char* device, int baud, char parity, int dataBit, int stopBit, int slaveId)
    : ctx(nullptr), device(device), baud(baud), parity(parity), dataBit(dataBit), stopBit(stopBit), slaveId(slaveId), connected(false) {
}
DDSU666Reader::~DDSU666Reader() {
    disconnect();
}
...
bool DDSU666Reader::connect() {
    ...
    if (ctx == nullptr) {
        std::cerr << "无法创建 libmodbus context: " << modbus_strerror(errno) << std::endl;
        return false;
    }
    modbus_set_slave(ctx, slaveId);
    if (modbus_connect(ctx) == -1) {
        std::cerr << "连接到 " << device << " 失败: " << modbus_strerror(errno) << std::endl;
        modbus_free(ctx);
        ctx = nullptr;
        return false;
    }
...
bool DDSU666Reader::isConnected() const {
    return connected;
}
...
bool DDSU666Reader::readFloatRegister(int regAddr, float& value) {
    ...
    if (modbus_read_registers(ctx, regAddr, 2, tabReg) == 2) {
        ...
        std::cerr << "读取寄存器 (0x" << std::hex << regAddr << std::dec << ") 失败: " << modbus_strerror(errno) << std::endl;
        return false;
    }
}
